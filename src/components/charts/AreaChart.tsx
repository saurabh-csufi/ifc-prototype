...
export function AreaChart({ series, width, height, unit, dataset }: AreaChartProps) {
  ...
  if (!series.length || series.every(s => s.observations.length === 0)) {
    return <div className="area-chart-wrapper">No data available</div>;
  }
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGRectElement>) => {
    ...
    setTooltip({ x: e.clientX, y: e.clientY, date, items });
  };

  const areaGenerator = d3.area<{ date: string; value: number }>()
    .x(d => xScale(d.date) || 0)
    .y0(yScale(yMin))
    .y1(d => yScale(d.value))
    ...
    <div className="area-chart-wrapper">
      ...
      <svg width={width} height={height}>
        <g transform={`translate(${MARGIN.left},${MARGIN.top})`}>
          ...
          {/* Area fills and lines per series */}
          {series.map((s, si) => {
            ...
            return (
              <g key={si}>
                <path
                  d={areaGenerator(sortedObs) || ''}
                  fill={color}
                  fillOpacity={series.length > 1 ? 0.25 : 0.35}
                />
                <path
                  d={lineGenerator(sortedObs) || ''}
                  fill="none"
                  stroke={color}
                  strokeWidth={2}
                />
                ...
                {sortedObs.map((obs, oi) => (
                  <circle
                    ...
                    cy={yScale(obs.value)}
                    r={hoverIdx !== null && allDates[hoverIdx] === obs.date ? 5 : 3}
                    fill={hoverIdx !== null && allDates[hoverIdx] === obs.date ? '#fff' : color}
                    stroke={color}
                    strokeWidth={2}
                  />
                ))}
